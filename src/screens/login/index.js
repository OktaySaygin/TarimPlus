...
const Login = (props) => {
  ...
  const handleSignUp = () => {
    fetchMyAPIPost("/login", generalState?.email, generalState?.password).then(response => {
      if (response.status) {
  ...
  return (
      <View style={{marginHorizontal: 30, flex: 1, justifyContent: 'center'}}>
        <Text size={22} type={'italicBold'}>TarımPlus'a Giriş Yap</Text>
        ...
        <TouchableOpacity
          ...
          <Text style={{color: 'white'}} size={18} type={'bold'}>Giriş Yap</Text>
        </TouchableOpacity>
